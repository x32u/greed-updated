...
class Filters:
    hyper_link = r"http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+"
    invite_link = r"(?:https?://)?discord(?:\.gg|app\.com/invite)/[a-zA-Z0-9]+/?"
    user_mention = r"<@!?\d+>$"
    default = [(hyper_link, ""), (invite_link, ""), (user_mention, "")]
...
def clean(
    ...
    return limit_repetitions(
        filter_patterns(string.lower(), filters).strip(), max_repetitions
    )
...
class Chains:
    ...
    def generate(self, bias: Optional[str] = None) -> str:
        if bias and bias in self.words:
            first = bias
        else:
            prefix_list = list(
                filter(lambda word: word not in self.punctuations, self.words.keys())
            )
        ...
        after = choices(suffix_list, suffix_weights)[0]
